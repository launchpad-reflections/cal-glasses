"""
Full flow: + → Scan food → Photo → explore photo picker
"""
import time
...
def dump(driver, label=""):
    ...
    for i, cell in enumerate(driver.find_elements(AppiumBy.CLASS_NAME, "XCUIElementTypeCell")[:10]):
        name = cell.get_attribute("label") or "(none)"
        print(f"  [{i}] {name}")
    print("\nText:")
